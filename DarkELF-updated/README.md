⚠️ This project is a modified version of [DarkELF](https://github.com/tongylin/DarkELF).  
It is based on the original code and has been extended/updated by [its631] on [2025/3/22].  
The main change is the replacement of the `interp2d` function with `CloughTocher2DInterpolator` to ensure compatibility with `pandas` version 2.2.3 and `scipy` version 1.9.3.  

This repository is intended for continued compatibility and educational/research use.  
All credits go to the original author. Please refer to the original repository for the latest official updates.
# DarkELF
