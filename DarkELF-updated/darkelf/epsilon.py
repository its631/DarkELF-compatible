##Modified interp2d to CloughTocher2DInterpolator
import numpy as np
from numpy import linspace, sqrt, array, pi, cos, sin, dot, exp, sinh, log, log10, cosh, sinh
from scipy.interpolate import interp1d, RegularGridInterpolator, CloughTocher2DInterpolator
...
def load_epsilon_grid(self,datadir,filename):
    ...
    data = pd.read_csv(fname, sep='\s+', header=None, skiprows=1,
                names=['omega', 'k', 'eps1', 'eps2'])

    data = data.bfill()# fill in some NaN values
    ...
    points = np.array([[o, k] for o in omall for k in kall])
    values_eps1 = eps1df.flatten()
    values_eps2 = eps2df.flatten()

    self.eps1_grid = CloughTocher2DInterpolator(points, values_eps1)
    self.eps2_grid = CloughTocher2DInterpolator(points, values_eps2)
